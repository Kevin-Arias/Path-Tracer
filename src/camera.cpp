...
Ray Camera::generate_ray_for_thin_lens(double x, double y, double rndR, double rndTheta) const {
  ...
  double interpolated_x = bottom_left.x + x * (top_right.x - bottom_left.x);
  double interpolated_y = bottom_left.y + y * (top_right.y - bottom_left.y);
  Vector3D d = Vector3D(interpolated_x, interpolated_y, -1);
  d.normalize();
  ...
  Vector3D pFocus = origin + (focalDistance*d);
  Vector3D direction = pFocus - pLens;
  direction.normalize();
  Ray final = Ray(c2w*pLens+pos, c2w*direction);
  final.min_t = nClip;
  final.max_t = fClip;
  return final;
